...
import SUBMIT_QUIZ_ENDPOINT from "./endpoints"
...
function submitQuiz(e) {
    ...
    let quizId = byId("quiz-id").value
    let materialId = byId("material-id").value
    ...
    let currQuizAnswers = Cache.answers.filter(a => a.id == quizId)[0]
    ...
    let allCorrectAnswers = correctAnswers == currQuizAnswers.answers.length
...
function sendQuizResults(payload, done) {
    ...
    return fetch(SUBMIT_QUIZ_ENDPOINT, init)
        .then(res => {
            if (res.ok) {
                res.json().then(json => {
...
function showQuizCompletionMessage(done) {
    let submitBtn = byId("submit-quiz"),
        materialId = byId("material-id").value,
        a = create("a");
...
export function setupQuiz() {
    ...
    let quizForm = byId("quiz-form")
    let materialId = byId("material-id").value
    let questionHeaders = bySelector("#quiz-container h3")
    let alternativeLists = bySelector("#quiz-container .alternatives")
